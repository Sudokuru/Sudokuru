...
function getCombinedScore(refutationScore:number, adjustedDependencyScore:number):number {
    return Math.round((refutationScore + adjustedDependencyScore) * 1000000);
}
...
function getCorrelationScores(puzzlesMetrics:PuzzleMetrics[], solveTimeSeconds:number[]):CorrelationScores{
    ...
    return {
        ...
        combinedScore: safeCalculateCorrelation(combinedScores, solveTimeSeconds)
    };
}
...
function printCorrelationScores(correlationScores:CorrelationScores) {
    ...
    console.log("Combined score correlation coefficient: " + correlationScores.combinedScore);
    console.log("");
}
...
for (let i:number = 0; i < data.length; i++) {
    ...
    averageCorrelationScores.combinedScore += correlationScores.combinedScore;
...
averageCorrelationScores.combinedScore /= data.length;
