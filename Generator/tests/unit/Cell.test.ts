...
describe("create Cell object", () => {
    ...
    it('should remove notes', () => {
        ...
        expect(obj.getNotes().has("1")).toBeTruthy;
        obj.removeNote("1");
        expect(obj.getNotes().has("1")).toBeFalsy;

        expect(obj.getNotes().has("2")).toBeTruthy;
        expect(obj.getNotes().has("3")).toBeTruthy;
        let notes:Map<string, undefined> = new Map([["2", undefined], ["3", undefined]]);
        obj.removeNotes(notes);
        expect(obj.getNotes().has("2")).toBeFalsy;
        expect(obj.getNotes().has("3")).toBeFalsy;
    });
});