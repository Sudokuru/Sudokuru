...
- Run only tests whose path is under `V4/`. Never run legacy `Generator/tests`, `npm test`, or an unscoped `bun test` command.
- Use Bun's built-in test runner. If `bun` is missing, stop and report it instead of installing or downloading code during verification.
- Use a focused command such as:

```text
bun test V4/tests/unit/<name>.test.ts
```
